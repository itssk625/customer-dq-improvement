...
def calculate_dashboard_metrics():
    ...
    total=cursor.fetchone()[0]
    cursor.execute(f"""select count(*) from final_customer_phone where cleaned_name is not null""", conn)
    valid_name_counts=cursor.fetchone()[0]
    cursor.execute(f"""select count(*) from final_customer_phone where cleaned_dob is not null""", conn)
    valid_dob_counts=cursor.fetchone()[0]
    cursor.execute(f"""select count(*) from final_customer_phone where cleaned_email is not null""", conn)
    ...
    cursor.execute(query, conn)
    valid_country_counts = cursor.fetchone()[0]
    cursor.execute(f"""select count(*) from final_customer_phone where gender is not null""", conn)
    valid_gender_counts=cursor.fetchone()[0]
    cursor.execute(f"""select count(*) from final_customer_phone where is_disposable_email""", conn)
    disposable_counts=cursor.fetchone()[0]
    disposable_pct=(disposable_counts/total)*100
