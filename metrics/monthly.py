...
def display_chart(df, title_, y_col, y_label, suffix="",auto_percent_range=False):
    ...
    if auto_percent_range:
        y_min=max(0, df[y_col].min()-2)
        y_max=min(100, df[y_col].max()+2)
...
def display_monthly_dashboard():
    ...
    display_chart(dq_trend, title_="Month-on-Month DQ Score", y_col='average_dq_score',y_label="DQ Score", suffix="%", auto_percent_range=True)
    ...
    display_chart(repo_trend, title_="Month-on-Month Record Growth", y_col='total_records', y_label='Total Records', auto_percent_range=False)
    ...
    if repo=="email":
        ...
        display_chart(phoneno_trend, title_="Month-on-Month Phone Number Validity Improvement", y_col='validity_pct', y_label='Phone Number Validity (%)', suffix="%", auto_percent_range=True)
    elif repo=="phone":
        email_trend=get_trend(conn, repo, "valid_email_count")
        display_chart(email_trend, title_="Month-on-Month Email Validity Improvement", y_col='validity_pct', y_label='Email Validity (%)', suffix="%", auto_percent_range=True)
    for column, title in charts:
        trend=get_trend(conn, repo, column)
        display_chart(trend, title_=f"Month-on-Month {title} Validity Improvement", y_col='validity_pct', y_label=f"{title} Validity (%)", suffix="%",auto_percent_range=True)
